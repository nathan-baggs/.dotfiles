...
-- Custom user options and keymaps
...
do
	...
	require("cmake-tools").setup({ cmake_kits_path = "~/.local/share/CMakeTools" })

	vim.pack.add({ gh("nvim-tree/nvim-tree.lua") })
	require("nvim-tree").setup({ view = { side = "right" }, hijack_netrw = false, disable_netrw = false })
	...
	pcall(function()
		require("obsidian").setup({
			...
			workspaces = {
				{
					name = "YouTube",
					path = "/mnt/c/Users/hiphish/OneDrive/Documents/Youtube",
				},
			},
		})
	end)
...
-- ============================================================
-- SECTION 8b: AI COMPLETION (minuet-ai)
-- Ghost-text FIM-style autocomplete via LiteLLM proxy
-- ============================================================
do
	...
	require("minuet").setup({
		...
		virtualtext = {
			-- Trigger automatically in all filetypes
			auto_trigger_ft = { "*" },
			auto_trigger_ignore_ft = { "TelescopePrompt", "ministarterstart", "gitcommit", "help" },
			keymap = {
				accept = false,
				accept_line = "<S-Tab>",
				next = "<M-]>",
				prev = "<M-[>",
				dismiss = "<M-e>",
				toggle_auto_trigger = "<leader>at",
			},
		},

		-- Disable blink auto-complete (we use virtualtext); keep manual trigger
		blink = { enable_auto_complete = false },

		provider_options = {
			openai_compatible = {
				name = "litellm",
				model = vim.env.MINUET_AUTOCOMPLETE_MODEL,
				end_point = (vim.env.OPENAI_API_HOST or "") .. "/v1/chat/completions",
...
vim.g.minuet_virtual_text_enabled = true -- tracks global toggle state; matches auto_trigger_ft = { '*' }

-- Augroup used to suppress minuet's own FileType autocmd when globally disabled.
-- Registered after minuet.setup(), so it fires last and wins.
local minuet_suppress_aug = vim.api.nvim_create_augroup("minuet_global_suppress", { clear = true })

local function minuet_apply_global_state(enabled)
	-- Flip the buffer-local var on all loaded buffers
	for _, buf in ipairs(vim.api.nvim_list_bufs()) do
		if vim.api.nvim_buf_is_loaded(buf) then
			vim.b[buf].minuet_virtual_text_auto_trigger = enabled
		end
	end
	-- When disabled: install a counter-autocmd that fires after minuet's FileType autocmd
	-- and overrides the buffer var back to false for any newly opened buffer.
	-- When enabled: clear that counter-autocmd so minuet's own autocmd takes effect again.
	vim.api.nvim_clear_autocmds({ group = minuet_suppress_aug })
	if not enabled then
		vim.api.nvim_create_autocmd("FileType", {
			group = minuet_suppress_aug,
			pattern = "*",
			callback = function()
				vim.b.minuet_virtual_text_auto_trigger = false
			end,
			desc = "minuet global disable: suppress auto trigger on new buffers",
		})
	end
end

-- Toggle minuet AI completions globally
vim.keymap.set("n", "<leader>ai", function()
	local new_state = not vim.g.minuet_virtual_text_enabled
	vim.g.minuet_virtual_text_enabled = new_state
	minuet_apply_global_state(new_state)
	vim.notify("Minuet virtual text " .. (new_state and "enabled" or "disabled") .. " globally", vim.log.levels.INFO)
end, { desc = "[A]I toggle [i]nline completions (global)" })

-- Smart Tab in insert mode: accept minuet suggestion if one is showing,
...
end, { desc = "Tab: accept minuet suggestion or indent", silent = true })
